...
open_r_df("parm.cons",1,8,2);
	...
	wrt_r_df_col("log_dm_cL_lc",log_dm_cL_lc_out);
	...
	wrt_r_df_col("log_dm_vll_ac",log_dm_vll_ac_out);
    ...
    wrt_r_df_col("selpar_A50_cL2",selpar_A50_cL2_out);
    wrt_r_df_col("selpar_A50_cL3",selpar_A50_cL3_out);	
    wrt_r_df_col("selpar_slope1_cL2",selpar_slope_cL2_out);
	wrt_r_df_col("selpar_A502_cL2",selpar_A502_cL2_out);
	wrt_r_df_col("selpar_slope2_cL2",selpar_slope2_cL2_out);
    ...
    wrt_r_df_col("selpar_A50_GR1",selpar_A50_GR1_out);
    wrt_r_df_col("selpar_slope_GR1",selpar_slope_GR1_out);
	...
	wrt_r_df_col("selpar_A501_vll",selpar_A50_vll_out);
    wrt_r_df_col("selpar_slope1_vll",selpar_slope_vll_out);
...
open_r_df("parm.tvec", styr, endyr, 2);
	...
	wrt_r_df_col("log.F.dev.cL", log_F_dev_cL_out);
...
open_r_df("parm.tvec.cons",1,3,2);
	...
	wrt_r_df_col("log.F.dev.cL",set_log_F_dev_cL);
	...
	wrt_r_df_col("log.q.dev.GR.RWq", set_log_RWq_dev);
close_r_df();
...
open_r_vector("sdnr");
	...
	wrt_r_item("sdnr.U.vll", sdnr_I_vll);
	
    wrt_r_item("sdnr.lc.cH", sdnr_lc_cH);
    wrt_r_item("sdnr.lc.cL", sdnr_lc_cL);	
	...
	wrt_r_item("sdnr.ac.vll", sdnr_ac_vll);
...
open_r_vector("like");
	...
	wrt_r_item("lk.U.vll", f_vll_cpue);
    wrt_r_item("lk.lenc.cH", f_cH_lenc);
    wrt_r_item("lk.lenc.cL", f_cL_lenc);	
	...
	wrt_r_item("lk.agec.vll", f_vll_agec);	
	...
	wrt_r_item("lk.U.GR.RWq", f_GR_RWq_cpue);
	...
	wrt_r_item("w.U.vll", w_I_vll);
	
    wrt_r_item("w.lc.cH", w_lc_cH);
    wrt_r_item("w.lc.cL", w_lc_cL);	
...
open_r_list("size.age.fishery");
    ...
    open_r_matrix("len.cL.mm");
    wrt_r_matrix(len_cL_mm, 2, 2);
    wrt_r_namevector(styr,endyr);
    wrt_r_namevector(agebins);
    close_r_matrix();
	...
	open_r_matrix("wholewgt.cL.lb");
    wrt_r_matrix(wholewgt_cL_klb*1000.0, 2, 2);
    wrt_r_namevector(styr,endyr);
    wrt_r_namevector(agebins);
    close_r_matrix();
...
open_r_list("sel.age");
	...
	open_r_matrix("sel.m.cL");
    wrt_r_matrix(sel_cL, 2, 2);
    wrt_r_namevector(styr,endyr);
    wrt_r_namevector(agebins);
    close_r_matrix();
	...
	open_r_matrix("sel.m.vll");
    wrt_r_matrix(sel_vll, 2, 2);
    wrt_r_namevector(styr,endyr);
    wrt_r_namevector(agebins);
    close_r_matrix();
...
open_r_list("comp.mats");
    ...
    open_r_matrix("lcomp.cL.ob");
    wrt_r_matrix(obs_cL_lenc, 2, 2);
    wrt_r_namevector(yrs_cL_lenc);
    wrt_r_namevector(lenbins);
    close_r_matrix();

    open_r_matrix("lcomp.cL.pr");
    wrt_r_matrix(pred_cL_lenc, 2, 2);
    wrt_r_namevector(yrs_cL_lenc);
    wrt_r_namevector(lenbins);
    close_r_matrix();
	...
	open_r_matrix("acomp.vll.ob");
    wrt_r_matrix(obs_vll_agec, 2, 2);
    wrt_r_namevector(yrs_vll_agec);
    wrt_r_namevector(agebins_agec);
    close_r_matrix();

    open_r_matrix("acomp.vll.pr");
    wrt_r_matrix(pred_vll_agec, 2, 2);
    wrt_r_namevector(yrs_vll_agec);
    wrt_r_namevector(agebins_agec);
    close_r_matrix();
...
open_r_df("t.series", styr, (endyr+1), 2);
	...
	wrt_r_df_col("U.vll.ob", obs_vll_cpue);
    wrt_r_df_col("U.vll.pr", pred_vll_cpue);
   	wrt_r_df_col("cv.U.vll", vll_cpue_cv/w_I_vll);
	wrt_r_df_col("cv.unwgted.U.vll", vll_cpue_cv);
	...
	wrt_r_df_col("q.vll", q_vll);
    wrt_r_df_col("q.vll.RW.log.dev",q_RW_log_dev_vll);
	...
	wrt_r_df_col("L.cL.ob", obs_cL_L);
    wrt_r_df_col("L.cL.pr", pred_cL_L_klb);
    wrt_r_df_col("cv.L.cL", cL_L_cv);
	...
	wrt_r_df_col("lcomp.cL.n", nsamp_cL_lenc_allyr);
	...
	wrt_r_df_col("acomp.vll.n", nsamp_vll_agec_allyr);

	wrt_r_df_col("lcomp.cH.nfish", nfish_cH_lenc_allyr);
	wrt_r_df_col("lcomp.cL.nfish", nfish_cL_lenc_allyr);
	//("lcomp.HB.nfish", nfish_HB_lenc_allyr);
	...
	wrt_r_df_col("acomp.vll.nfish", nfish_vll_agec_allyr);
	
	wrt_r_df_col("lcomp.cH.neff", (1+nsamp_cH_lenc_allyr*exp(log_dm_cH_lc_out(8)))/(1+exp(log_dm_cH_lc_out(8))) );
	wrt_r_df_col("lcomp.cL.neff", (1+nsamp_cL_lenc_allyr*exp(log_dm_cL_lc_out(8)))/(1+exp(log_dm_cL_lc_out(8))) );
    ...
    wrt_r_df_col("acomp.vll.neff", (1+nsamp_vll_agec_allyr*exp(log_dm_vll_ac_out(8)))/(1+exp(log_dm_vll_ac_out(8))) );  	
...
open_r_df("LD.pr.tseries", styr, endyr, 2);
    ...
    wrt_r_df_col("L.cL.klb", pred_cL_L_klb);
    wrt_r_df_col("L.cL.knum", pred_cL_L_knum);	
